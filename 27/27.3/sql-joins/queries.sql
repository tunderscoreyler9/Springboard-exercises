-- # 1 Join the two tables so that every column and record appears, regardless of if there is 
-- not an owner_id
SELECT * FROM owners FULL JOIN vehicles ON owners.id = vehicles.owner_id;

-- # 2 Count the number of cars for each owner. Display the owners first_name, last_name and
